...
for (j in seq_along(variations)) {
  ...
  csv_mslpr$n_reqs <- substr(csv_mslpr$group, 1, 2)
  ...
  output_filename <- paste0("table_counts_obs_per_n_reqs_and_instance_lp_impr_", variations[j], "_mslp.csv")
  file_path <- file.path(
  ...
  output_filename <- paste0("plot_feas_sol_perc_vs_n_reqs_and_instance_", variations[j], "_mslp.pdf")
  file_path <- file.path(
  ...
  output_filename <- paste0("plot_mean_lp_impr_vs_n_reqs_and_instance_", variations[j], "_mslp.pdf")
  file_path <- file.path(
      prefix_output_path_plots,
      "n_reqs",
      output_filename
