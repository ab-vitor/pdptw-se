...
output_dir_plots <- "official_experiments/solution_chars/plots/n_regions"
output_dir_tables <- "official_experiments/solution_chars/tables/n_regions"
...
for (j in seq_along(variations)) {
  ...
  for (l in seq_along(method_names)){
    ...
    obs_counts <- csvr_df %>%
      filter(feasible == 1) %>%
      group_by(type, n_regions) %>%
      summarise(count = n(), .groups = "drop") %>%
      arrange(type, n_regions)
    
    file_name <- paste0("obs_plots_vs_n_regions_and_instance_", method_names[l], "_", var, ".csv")
    ...
    for (i in seq_along(columns_sol_chars)) {
      ...
      p <-
        ggplot(
          ...
          aes_string(
            ...
            group = interaction(csvr_df$type, csvr_df$n_regions),
            fill = "n_regions"
          )
        ) +
        geom_boxplot() +
      ...
      file_name <- paste0("plot_", columns_sol_chars[i], "_vs_n_regions_and_instance_", method_names[l], "_", var, ".pdf")
