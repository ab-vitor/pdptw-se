...
for (j in 1:2) {
  ...
  p <- ggplot(csvr_mip_mslp, aes(x = type, y = dev_mip_mslp_minbs, fill=n_reqs, group = as.factor(interaction(type, n_reqs)))) +
    geom_boxplot() +
    # geom_point(
    #   aes(x = type, y = dev_mip_mslp_minbs, grouop = interaction(type, n_reqs)),
    #   shape = 21,
  ...
  ggsave_filename <- paste0("box_plots_sol_dev_mip_mslp_", var, "_by_n_reqs.pdf")
