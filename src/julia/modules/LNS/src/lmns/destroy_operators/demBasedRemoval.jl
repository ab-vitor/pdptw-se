function demandBasedRemoval!(extld::ExternalLMNSData, inst::InstanceData, allParams::AllParams)::Nothing
	idop = Int(extld.destroyOperator)
	psi = extld.psi[idop]
	mipModel = extld.mipModel
	ddp = allParams.dgd

	println("-----------## Performing demand-based removals | psi = $(psi) ##------------")
	...
	for _ in 1:psi-1
		i = jobsToRemove[end]
		jobDemValues = Tuple{Float64, Int64, Int64}[]
		for jidx in eachindex(jobsToAnalyze)
			j = jobsToAnalyze[jidx]
			demDiff = abs(inst.q[i] - inst.q[j])
			demValue = demDiff
			push!(jobDemValues, (demValue, j, jidx))
		end
		idxArgMin = argmin(jobDemValues)
		demValue, j, jidx = jobDemValues[idxArgMin]
